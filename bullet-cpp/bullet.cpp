...
Object Bullet::get_collider(Dictionary& result)
{
    return ObjectDB::get_instance(*Object::cast_to<uint64_t>(result["collider_id"]));
}
...
void Bullet::_physics_process(double delta)
{
    ...
    if (tick)
    {
        index_start = index_halt;
        index_halt = 0;
    }
    while(index_start < index_halt)
    {
